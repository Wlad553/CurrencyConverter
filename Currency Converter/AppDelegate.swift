...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func handleAppRefreshTask(task: BGProcessingTask) {
        ...
        mainViewController.networkCurrenciesDataManager.fetchCurrencyRatesData(urlSession: backgroundURLSession)
...
extension AppDelegate: URLSessionDataDelegate {
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        guard let currencyParsedData = try? mainViewController.networkCurrenciesDataManager.parseJSON(withRatesData: data) else { return }
        let coreDataManager = mainViewController.coreDataManager
