...
final class CurrencyConverterUITests: XCTestCase {
    ...
    func testMainScreenElementsExistence() {
        ...
        XCTAssertTrue(app.staticTexts["appName"].exists)
        XCTAssertTrue(app.buttons["askButton"].exists)
        XCTAssertTrue(app.buttons["bidButton"].exists)
        XCTAssertTrue(app.otherElements["mainWindowView"].exists)
        XCTAssertTrue(app.buttons["addCurrencyButton"].exists)
        XCTAssertTrue(app.staticTexts["lastUpdatedLabel"].exists)
        XCTAssertTrue(app.staticTexts["lastUpdatedTimeLabel"].exists)
        XCTAssertTrue(app.tables["mainWindowViewTableView"].exists)
        XCTAssertTrue(app.buttons["shareButton"].exists)
        
        let firstCell = app.tables["mainWindowViewTableView"].cells.firstMatch
    ...
    func testMoveRowsMainScreenTableView() {
        ...
        firstCell.staticTexts.firstMatch.press(forDuration: 1.2)
    ...
    func testDeleteAndAddCell() {
        ...
        firstCell.buttons["trash"].tap()
        XCTAssertFalse(app.staticTexts[firstCellLabelText].exists)
        
        app.buttons["addCurrencyButton"].tap()
        XCTAssertTrue(app.tables["addCurrencyTableView"].exists)
        
        let addCurrencyScreenFirstCell = app.tables["addCurrencyTableView"].cells.firstMatch
        ...
        XCTAssertFalse(app.tables["addCurrencyTableView"].exists)                
        XCTAssertTrue(app.staticTexts[addedCurrencyCode].exists)
    }
    
    func testAddCurrencyScreen() {
        app.buttons["addCurrencyButton"].tap()
        let tableView = app.tables["addCurrencyTableView"]
